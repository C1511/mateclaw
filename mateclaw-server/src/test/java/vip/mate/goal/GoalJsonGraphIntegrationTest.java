...
class GoalJsonGraphIntegrationTest {
    ...
    void realGraphsPreserveAuthenticatedOriginThroughReadPublishCheckAndComplete(boolean plan, boolean scheduled) throws Exception {
        ...
        when(evaluator.evaluate(any(), anyList(), anyString())).thenReturn(GoalEvaluationResult.fallback("offline_graph_fixture"));
        ...
        org.mockito.stubbing.Answer<ChatResponse> script = invocation -> {
            ...
            switch (step) {
                ...
                case 4 -> { assertTrue(last.path("acceptanceEligible").asBoolean(), String.valueOf(last)); name = "completeGoal"; }
                default -> {
        ...
        assertTrue(calls.get() >= 6 && calls.get() <= 10, "Bounded scripted model calls: " + calls.get());
    }
}
